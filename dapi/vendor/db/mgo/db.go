...
func (db *Database) register(name string, url string, username string, password string) *Database {
	if db.registered {
...
func Register(dbID, dbName string, url string, username string, password string) *Database {
	return GetDB(dbID).register(dbName, url, username, password)
}
...
func (db *Database) reconnect() {
	...
	if len(strings.Trim(db.username, " ")) > 0 && len(strings.Trim(db.password, " ")) > 0 {
...
func (db *Database) DialWithInfo(info *mgo.DialInfo) (*mgo.Session, error) {
