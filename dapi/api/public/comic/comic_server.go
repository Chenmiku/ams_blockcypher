...
func NewComicServer() *ComicServer {
	...
	s.HandleFunc("/get_by_type", s.mustGetComicByType)
	return s
}
...
func (s *ComicServer) mustGetComicByType(w http.ResponseWriter, r *http.Request) {
	sortBy := r.URL.Query().Get("sort_by")
	sortOrder := r.URL.Query().Get("sort_order")
	...
	arrayComic, err = comic.GetByType(pageSize, pageNumber, sortBy, sortOrder, types)
